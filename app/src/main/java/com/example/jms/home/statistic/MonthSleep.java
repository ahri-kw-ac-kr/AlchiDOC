...
public class MonthSleep extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        for(int i=0; i<monthList.size(); i++){
            ...
            try{deepPercentList[i] = (int) ((((double)monthList.get(i).getDeep() / (double)monthList.get(i).getTotal())) * 100);}catch (Exception e){deepPercentList[i]=0;}
        }
        ...
        for(int i=0; i<thisDay; i++){
            if(percentList[i]>=80&&deepPercentList[i]>=25){ perfect+=1; }
            else if((percentList[i]>=80&&deepPercentList[i]<25)||(percentList[i]<80&&deepPercentList[i]>=25)){ soso+=1; }
            else if(percentList[i]<80&&deepPercentList[i]<25){ lack+=1; }
        }
